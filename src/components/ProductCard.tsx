...
export default function ProductCard({ product }: { product: ProductLike }) {
  ...
  return (
    <div className="group relative bg-white border border-gray-200 rounded-2xl p-4 hover:shadow-xl transition-all duration-300 flex flex-col h-full hover:border-transparent">
      ...
      {/* Image */}
      <Link href={product.frontend_url ? `/${product.frontend_url.replace(/^product\//, '')}` : `/${product.url || product.id}`} className="relative block w-full aspect-[1/1] mb-4">
        {imgUrl ? (
          <Image
            src={imgUrl}
            alt={product.name ?? ""}
      ...
      {/* Title */}
      <Link href={product.frontend_url ? `/${product.frontend_url.replace(/^product\//, '')}` : `/${product.url || product.id}`} className="block mb-3">
