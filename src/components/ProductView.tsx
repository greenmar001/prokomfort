...
export default function ProductView({ product: p, categories: allCats }: ProductViewProps) {
    // Build crumbs
    ...
    if (cat) {
        if (cat.parent_id) {
            ...
            if (parent) {
                // Use full_url if available for parents, basically ensuring we link to correct nested cat
                const parentUrl = parent.full_url ? `/${parent.full_url}` : (parent.url ? `/${parent.url}` : `/category/${parent.id}`);
                crumbs.push({ title: parent.name, href: parentUrl });
            }
        }
        const catUrl = cat.full_url ? `/${cat.full_url}` : (cat.url ? `/${cat.url}` : `/category/${cat.id}`);
        crumbs.push({ title: cat.name, href: catUrl });
    }
